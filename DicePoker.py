...
while ((play_again == 'y') and (rol.score > 9)):
	...
	while not((second_chance == 'y') or (second_chance == 'n')):
		second_chance = input('Do you want to use your second chance? "y" for yes, "n" for no: ')
		...
		if second_chance == 'y':
			...
			for i in range(roll_num):
				changes = int(input('Enter value you wish to change, to skip 1 dice roll enter "0": '))
	...
	# Prompts user to continue playing or quit playing after each round of playing
		...
		play_again = input('Do you want to play again? "y" for yes, any character to stop: ')
